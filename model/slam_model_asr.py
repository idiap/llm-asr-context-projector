...
#   - Added the context projector: created when dataset_config.enable_context_projector is set,
#     loaded from ctx_ckpt, and frozen with train_config.freeze_context_projector
...
def model_factory(train_config, model_config, **kwargs):
    ...
    if kwargs.get("dataset_config", {}).get("enable_context_projector", False):
        context_projector = setup_context_projector(
            train_config, model_config, **kwargs
        )
    else:
    ...
    # Load context projector if checkpoint provided
    ctx_path = kwargs.get(
        "ctx_ckpt", None
    )
    if ctx_path is not None:
        logger.info("Loading Context Projector from: {}".format(ctx_path))
        ckpt_dict = torch.load(ctx_path, map_location="cpu")
        model.load_state_dict(ckpt_dict, strict=False)
    ...
    # Freeze context projector if specified in config
    if train_config.freeze_context_projector and context_projector is not None:
        for _, param in model.context_projector.named_parameters():
                param.requires_grad = False
