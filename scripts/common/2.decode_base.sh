...
#
# Decode the test set with a model trained by 1.finetune_base.sh (METHOD=base, raw_kw or raw_ctx).
...
case "$METHOD" in
    base|raw_kw|raw_ctx) ;;
    *) echo "ERROR: 2.decode_base.sh decodes METHOD=base, raw_kw or raw_ctx, not '$METHOD'" >&2; exit 1 ;;
esac

cd $RUN_DIR

output_dir=$RUN_EXP_DIR/${BASE_CKPT_FOLDER}
