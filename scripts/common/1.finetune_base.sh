...
#
# Train a speech projector with the encoder and the LLM frozen.
#   METHOD=base    : empty context (the base model every context projector is trained on)
#   METHOD=raw_kw  : keywords of the previous turns inserted in the prompt as raw text
#   METHOD=raw_ctx : transcripts of the last CTX previous turns inserted in the prompt as raw text
...
case "$METHOD" in
    base|raw_kw|raw_ctx) ;;
    *) echo "ERROR: 1.finetune_base.sh trains METHOD=base, raw_kw or raw_ctx, not '$METHOD'" >&2; exit 1 ;;
esac
...
if [ "$RAW_TEXT_CONTEXT" = "true" ]; then
    context_args="++dataset_config.enable_context_projector=false \
