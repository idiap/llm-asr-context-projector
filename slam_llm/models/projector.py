...
#   - Added ContextProjector, which maps sentence-level dialogue-history embeddings
...
class ContextProjector(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.context_dim = config.context_dim # dimension of the input context embeddings
        self.llm_dim = config.llm_dim
        self.linear1 = nn.Linear(self.context_dim, 2048)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(2048, config.llm_dim)
        self.model_config = config
