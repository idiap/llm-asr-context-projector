...
#   - Added the context projector, which maps dialogue-history embeddings to LLM input
#     embeddings placed at the context-token positions of the prompt
...
def model_factory(train_config, model_config, **kwargs):
    ...
    # Context projector
    if kwargs.get("dataset_config", {}).get("enable_context_projector", False):
        context_projector = setup_context_projector(train_config, model_config, **kwargs)
    else:
...
def setup_context_projector(train_config, model_config, **kwargs):
    from slam_llm.models.projector import ContextProjector

    context_projector = ContextProjector(model_config)
    print_module_size(
        context_projector,
        "context_projector",
        (
...
class slam_model(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        context_embeddings = kwargs.get("context_embeddings", None)
        ...
        context_outs = None
        if context_embeddings is not None and self.context_projector is not None:
            if self.train_config.freeze_context_projector:  # the context projector is frozen
                self.context_projector.eval()
            context_outs = self.context_projector(context_embeddings)
        if (
        ...
        if input_ids is not None:
            input_ids[(input_ids == -1) | (input_ids == -2)] = 0  # -1: speech, -2: context placeholders
        ...
        if context_mask is not None and context_outs is not None:
            context_mask_start_indices = (context_mask == True).float().argmax(dim=1)
            context_lengths = torch.clamp(
                context_mask.sum(dim=1), max=context_outs.shape[1]
            ).tolist()

            context_outs_pad = torch.zeros_like(inputs_embeds)
            for i in range(context_outs.shape[0]):
                context_outs_pad[
                    i,
                    context_mask_start_indices[i] : context_mask_start_indices[i]
                    + context_lengths[i],
                ] = context_outs[i][: context_lengths[i]]

            inputs_embeds = context_outs_pad + inputs_embeds * (
                ~context_mask[:, :, None]
            )
